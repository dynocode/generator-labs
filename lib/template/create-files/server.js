...
function createServerIndex(that, serverPath, templateData) {
  ...
  return [
    ...
    {
      start: `nodemon -r dotenv/config ${path.join(relativeRootPath, 'index.js')}`,
      serve: `node -r dotenv/config ${path.join(relativeRootPath, 'index.js')}`,
      dev: `nodemon --exec babel-node -r dotenv/config ${path.join(relativeRootPath, 'index.js')}`,
    },
  ];
}
